...
def main():
    ...
    # uf = URISFactoryHandler('http://localhost:9326/', 'hercules.org', 'um', 'en-EN')
    # dc = DataGenerator(open('./data/instances_generator_metadata.json'),uf)
    # th = TrellisHandler('localhost', '8080', None)
    # dc.generate_sintetic_data()
    # properties = dc.get_properties()
    # th.create_basic_container(None, 'properties')
    # for p in properties:
    #     th.create_property('properties',p)
    # bash.stop_environment()
